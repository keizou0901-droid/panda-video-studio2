...
const CHECKPOINT_FOLDER = '03_subtitles';
...
const main = () => {
  ...
  const textTrackCounts = {};

  for (const [sceneId, scene] of Object.entries(currentVideo.scenes ?? {})) {
    textTrackCounts[sceneId] = Array.isArray(scene.text_tracks)
  ...
  console.log('text_tracks retained');
  for (const [sceneId, count] of Object.entries(textTrackCounts)) {
    console.log(`${sceneId}: ${count}`);
  }
};
